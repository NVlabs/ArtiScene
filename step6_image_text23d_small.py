...
""" Just to parallelize 3d asset generation by running separately for small objects. """
...
if __name__ == "__main__":
    ...
    while all_folders:
        ...
        for folder in all_folders:
            print(folder)
            args.json_path=os.path.join(folder,'for_image_text23d/small_objects_merged_12/small_objects_cropped_seg_rgb_together_merged_12.json')
            args.image_folder=os.path.join(folder,'for_image_text23d/small_objects_merged_12/images')
            try:
                ##################
                with open(Path(args.json_path)) as f:
                    objectname2description = json.load(f)
                # if there is a repeat.json that records which objects are predcited to be of the same model, it will be read to reduce repetitive generations.
                if 'repeat.json' in os.listdir(os.path.join(folder,'for_image_text23d/small_objects_merged_12')):
                    with open(os.path.join(folder,'for_image_text23d/small_objects_merged_12/repeat.json')) as f:
                        repeat = json.load(f)
                else:
    ...
    print('time2', time2)
    with open(os.path.join(folder,'step6_gen3d_small_time.txt'), 'w') as f:
        f.write(str(time2))
